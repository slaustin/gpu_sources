...
int main ()
{
  int blocks,threads,line_num,frame,i,k,j,x,y,z,points,grid_point,min_x,min_y,min_z,max_x,max_y,max_z,max_frame,line_counter,avg_only,print_flg,all_points,test_sum,pare;
...
while (fgets(buf, sizeof (buf), file)) {
       if(line_counter==all_points){
		if(frame%1==0){printf("GPU Run on Frame=%i\n",frame);fflush(stdout);}
		CHECK (cudaMemcpy(dev_covariance, covariance, (sqpoints)*sizeof(float), cudaMemcpyHostToDevice) );
		CHECK (cudaMemcpy(dev_variance, variance, points*sizeof(float), cudaMemcpyHostToDevice) );
	...
	for (k=0;k<points;k++){
		for (j=0;j<=k;j++){
			...
			for (i=1;i<=j;i++){pare+=i;}
				...
				if(j!=k){fprintf(ofp2,"%i\t%i\t%f\n",j+1,k+1,(covariance[grid_num]/covariance_2[grid_num]));}
		}
	}
fclose(ofp2);
}
