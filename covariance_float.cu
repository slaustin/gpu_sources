...
int main ()
{
  int blocks,threads,line_num,frame,k,j,x,y,z,points,grid_point,min_x,min_y,min_z,max_x,max_y,max_z,max_frame,line_counter,avg_only,print_flg,all_points,test_sum;
  unsigned long long int sqpoints,grid_num;
  int devCount;
  float bias,R,T,count;
  int *reso;
...
while (fgets(buf, sizeof (buf), file)) {
	if(line_counter==all_points){
		...
		grid_point=0;
		line_counter=0;}
	sscanf (buf, "%i\t%f\t%f",&frame,&count,&bias);
...
while (fgets(buf, sizeof (buf), file)) {
       if(line_counter==all_points){
		...
		//CHECK (cudaMemcpy(variance, dev_variance, points*sizeof(float), cudaMemcpyDeviceToHost) );
		grid_point=0;
		line_counter=0;}
        if(reso[line_counter]==1){
        	sscanf (buf, "%i\t%f\t%f",&frame,&count,&bias);
...
//for (k=0;k<(points*points);k++){
//	printf("covariancek=%f\n",covariance[k]);
//}
...
for (k=0;k<points;k++){
	for (j=0;j<points;j++){
		...
		grid_num+=k;
		//printf("%llu\n",grid_num);
		//fprintf(ofp2,"%i\t%i\t%llu\n",k+1,j+1,grid_num);
		//if(covariance_2[grid_num]!=0.00){ 
